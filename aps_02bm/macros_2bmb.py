...
def change2White():
    ...
    time.sleep(3)                
    epics.caput(BL+":Slit1Hcenter.VAL",2.8, wait=True, timeout=1000.0)
    epics.caput(BL+":m7.VAL",3.55, wait=True, timeout=1000.0)                
...
#    epics.caput(BL+":m33.VAL",-79.8, wait=False, timeout=1000.0)                    
    ...
    time.sleep(1)                                            
    epics.caput(BL+":m26.VAL",-0.4, wait=False, timeout=1000.0)    
    epics.caput(BL+":m27.VAL",-0.4, wait=False, timeout=1000.0)    
    epics.caput(BL+":m29.VAL",-0.4, wait=True, timeout=1000.0)                
    time.sleep(3)
    epics.caput(BL+":m25.VAL",80, wait=False, timeout=1000.0)    
    epics.caput(BL+":m28.VAL",80, wait=True, timeout=1000.0)
    time.sleep(3)                
    epics.caput(BL+":Slit1Hcenter.VAL",4.6, wait=True, timeout=1000.0)    
...
def change2Pink():
    ...
    BL = '2bma'
    epics.caput(shutter+":close.VAL",1, wait=True, timeout=1000.0)
#    epics.caput(BL+":m33.VAL",-77.8, wait=False, timeout=1000.0)                    
    epics.caput(BL+":fltr1:select.VAL",0, wait=True, timeout=1000.0)
    epics.caput(BL+":M1avg.VAL",0, wait=True, timeout=1000.0)
    time.sleep(1)                    
    epics.caput(BL+":M1angl.VAL",2.657, wait=True, timeout=1000.0)
...
#        return 0
    caliEng_list = np.array([31.0,27.4,24.9,22.7,21.1,20.2,18.9,17.6,16.8,16.0,15.0,14.4])
    XIASlit_list = np.array([42.35,44.35,46.35,46.35,47.35,48.35,50.35,50.35,52.35,53.35,54.35,55.35])    
#    FlagSlit_list = np.array([19.9,,19.47])    
#    GlobalY_list = np.array([-87.9,1.15,-79.8,1.25,-84.2,1.35,1.4,1.45,1.5,-79.8,1.6,1.65])                    
    USArm_list = np.array([1.1,1.15,1.2,1.25,1.3,1.35,1.4,1.45,1.5,1.55,1.6,1.65])    
    DSArm_list = np.array([1.121,1.169,1.2205,1.271,1.3225,1.366,1.4165,1.4655,1.5165,1.568,1.6195,1.67])
    M2Y_list = np.array([12.07,13.11,14.07,15.07,15.67,16.87,17.67,18.47,19.47,20.57,21.27,22.27])        
...
def DimaxMultiScan(exposureTime=0.0004, slewSpeed=360,angEnd = 180,
                numProjPerSweep=1000, repeats = 21,
                samInPos=0, samOutPos=12,
                roiSizeX = 2016, roiSizeY = 600,
                hutch = 'A',
                delay=0, accl = 180.0):
...
def DimaxMultiPosScan(exposureTime=0.025, slewSpeed=4,angEnd = 180,
                numProjPerSweep=1500,
                samInPos=0, samOutPos=12,
                roiSizeX = 2016, roiSizeY = 600,
                posInit = 9.3, posStep = 3, posNum = 8, axisShift = 12.0, refYofX0 = 23.46925,
                delay = 0,
                accl = 15):
...
#    epics.caput(shutter + ":open.VAL", 1, wait=True, timeout=1000.0)
    ...
    numImage = (numProjPerSweep+1)*posNum
...
def EdgeRadiography(exposureTime=0.01, frmRate = 60, acqPeroid = 30, 
                    roiSizeX = 2560, roiSizeY = 1080,scanMode=0):
...
def EdgeSingleScan(exposureTime=0.001, slewSpeed=10,angEnd = 180,
                numProjPerSweep=1500,
                samInPos=-0.0, samOutPos=0,
                roiSizeX = 2560, roiSizeY = 2160,
                accl = 5, shutterMode=0, scanMode=2):
    """
        Customize for Brian Patterson in situ loading experiments.
    """                
###### AHutch tomo configurations -- start
...
####### BHutch tomo configurations -- end    
    ...
    scanDelta = 1.0*angEnd/numProjPerSweep
    acclTime = 1.0*slewSpeed/accl
    frate = int(1.0*numProjPerSweep/(1.0*angEnd/slewSpeed) + 50)                
...
#def EdgeMultiPosScan(exposureTime=0.005, slewSpeed=18, angStart=0, angEnd = 180,
#                numProjPerSweep=1501,
#                samInPos=21.377, samOutPos=-4,
#                roiSizeX = 2560, roiSizeY = 900,
#                posInit = 0, posStep = 0, posNum = 1, axisShift = 0.0,refYofX0 = 22.772,
#                delay = 0, flatPerScan = 1, darkPerScan = 1,
#                accl = 9, scanMode=0, clShutter=1):
def EdgeMultiPosScan(exposureTime=0.02, slewSpeed=6, angStart=0, angEnd = 180,
                numProjPerSweep=751,
                samInPos=0, samOutPos=1,
                roiSizeX = 1280, roiSizeY = 2160,
                posStep = 0, posNum = 1, axisShift = 0.0,refYofX0 = 22.772,
                delay = 0, flatPerScan = 1, darkPerScan = 1,
                accl = 3, shutterMode=0, scanMode=0, clShutter=1):    
...
#    epics.caput(camPrefix+":cam1:FrameType.VAL",'FlatField', wait=True, timeout=1000.0) 
    ...
    epics.caput(camPrefix+":cam1:NumImages.VAL","10", wait=True, timeout=1000.0)
#    print epics.caget(camPrefix + ":cam1:SizeX_RBV.VAL"),epics.caget(camPrefix + ":cam1:SizeY_RBV.VAL")                
    epics.caput(camPrefix+":cam1:Acquire.VAL","Acquire", wait=False, timeout=1000.0)
    if epics.caget(camPrefix+":cam1:DetectorState_RBV.VAL") != 1:
        time.sleep(1)    
...
def _dimaxAcquisitionWODump(samInPos,samStage,numProjPerSweep,shutter,PSO = "2bmb:PSOFly1",rotStage="2bmb:m82"):
    ...
    epics.caput(camPrefix + ":cam1:FrameType.VAL",0, wait=True, timeout=1000.0)                 
    epics.caput(samStage+".VAL",str(samInPos), wait=True, timeout=1000.0)
    epics.caput(rotStage + ".VELO","360.00000", wait=True, timeout=1000.0)
    epics.caput(rotStage + ".VAL","0.00000", wait=False, timeout=1000.0)                
    epics.caput(PSO+":taxi.VAL","Taxi", wait=True, timeout=1000.0)
    epics.caput(PSO+":fly.VAL","Fly", wait=True, timeout=1000.0)                    
#    epics.caput(shutter+":close.VAL",1, wait=True, timeout=1000.0)
